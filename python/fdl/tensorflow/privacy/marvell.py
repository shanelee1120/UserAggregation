...
https://github.com/bytedance/fedlearner/tree/master/example/privacy/label_protection
'''
...
def marvell_perturb_fn(g, batch_y, 
                       uv_choice="uv", init_scale=1.0, 
                       dynamic=False, sumKL_threshold=0.25, 
                       p_frac='pos_frac', max_trials=100):
    logging.info(
        f"init_scale[{init_scale}] "
        f"uv_choice[{uv_choice}] "
        f"dynamic[{dynamic}]"
        f"sumKL_threshold[{sumKL_threshold}] "
        f"p_frac[{p_frac}] "
        f"max_trials[{max_trials}]")
    
    batch_y = tf.reshape(tf.cast(batch_y, dtype = tf.float32), [-1, 1])
    ...
    def compute_lambdas(u, v, scale, d, g_norm_square, p, sumKL_threshold, pos_g_mean, neg_g_mean, g_diff):
        ...
        d = np.float32(d.item())
        g_norm_square = np.float32(g_norm_square.item())
        p = np.float32(p.item())
        sumKL_threshold = np.float32(sumKL_threshold.item())
        ...
        while True:
            ...
            if not dynamic or sumKL <= sumKL_threshold:
                break
            num_trials += 1
            if max_trials is not None and max_trials > 0 and num_trials > max_trials:
                logging.warn(f'solve_isotropic_covariance early stops at {max_trials} iterations, sumKL = {sumKL}')
                break
